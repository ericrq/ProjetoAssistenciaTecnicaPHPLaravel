...
@section('content')
    ...
    <section class="form">
        ...
        <form action="/update/{{ $client->id }}" method="post">
            ...
            <input class="textos" type="text" name="name" id="nome" required value="{{ $client->name }}"><br>

            <label for="telefone" class="textos">Telefone</label>
            <input class="textos" type="tel" name="phone" id="telefone" required value="{{ $client->phone }}"><br>

            <label for="endereço" class="textos">Endereço</label>
            <input class="textos" type="text" name="address" id="endereco" required value="{{ $client->address }}"><br>

            <label for="cpf" class="textos">CPF</label>
            <input class="textos" type="text" name="cpf" id="cpf" required value="{{ $client->cpf }}"><br>

            <label for="email" class="textos">Email</label>
            <input class="textos" type="email" name="email" id="email" required value="{{ $client->email }}"><br>

            <label for="defeito" class="textos">Defeito</label>
            <textarea class="textos" name="defect" id="defeito" cols="30" rows="10"
                placeholder="Descreva o Defeito Do Equipamento" required>{{ $client->defect }}</textarea><br>
